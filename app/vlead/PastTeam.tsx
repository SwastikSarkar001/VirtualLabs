...
export default function PastTeam() {
  return (
    <div className="mx-auto max-w-[100rem] self-stretch px-4 md:px-16">
      <h1
        className={`text-4xl mb-12 font-bold text-center py-5 ${montserrat.className}`}
      >
        Past Team
      </h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-16 justify-items-center py-20 mb-16">
        {
					logos.map((logo) => (
						...
						>
							...
							<h2 className="mt-10 text-2xl font-bold text-center text-white">
								{logo.name}
							</h2>
							<p className="text-lg font-medium text-center text-white">
								{logo.subheading}
							</p>
							<p className="text-center mx-2 py-2 line-clamp-4 text-white">{logo.text}</p>
							<button className="mt-7 bg-violet-000 hover:bg-violet-900 outline-solid outline-purple-900 text-white font-bold py-2.5  px-8 rounded-full transition">
								{logo.buttonText}
							</button>
						</div>
					))
